...
class PurchaseConfirmationController extends AbstractController
{
    ...
    public function confirm(Request $request)
    {
        ...
        // 1 lire les données de formulaire  :     // FormFactoryInterface /Request
        $form = $this->createForm(CartConfirmationType::class);
        $form->handleRequest($request);

        // 2 Si le formualaire n'a pas été soumis : quitter
        if (!$form->isSubmitted()) {
        ...
        // 3. Si je ne suis pas connecté : quitter (security)
        $user = $this->getUser();

        // 4. si il n(y a pas de produits dans mon panier : quitter)
